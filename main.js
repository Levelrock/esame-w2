...
function cerca(e) {
  e.preventDefault()
  let titolo = document.querySelector("input#title").value
  let location = document.querySelector("input#location").value
  let list = document.getElementById("list")
  list.innerHTML = ''
  const stampa = search(titolo, location)
  console.log(stampa);

  for (let i = 0; i < stampa.result.length; i++) {
    const work = stampa.result[i];
    list.innerHTML += `<li>${work}</li>`
  }
}

document.getElementById("bottone").addEventListener('click', cerca)